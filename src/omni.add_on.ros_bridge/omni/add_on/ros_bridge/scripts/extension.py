...
class Extension(omni.ext.IExt):
    def on_startup(self, ext_id):
        ...
        sys.path.append(os.path.join(self._extension_path, "omni", "add_on", "ros_bridge", "packages"))

        self._rosbridge = _ros_bridge.acquire_ros_bridge_interface(ext_id)
        
    def on_shutdown(self):
        if self._extension_path is not None:
            sys.path.remove(os.path.join(self._extension_path, "omni", "add_on", "ros_bridge", "packages"))
            self._extension_path = None
